...
Each snapshot is a directory containing a sanitized `index.html`, a
`manifest.json`, and a `resources/` sidecar directory for CSS, fonts,
images, and the screenshot. The format is frozen as **v2** in
...
│   │   ├── manifest.json           # Metadata, manifest.version = 2
│   │   └── resources/              # Sidecar files referenced by index.html
│   │       ├── screenshot.webp     # Visual reference (or .png)
│   │       ├── <sha1>.css          # Stylesheet sidecars
│   │       └── <sha1>.woff2        # Fonts, images, etc. (trace bundles)
│   └── error/
│       ├── index.html
│       ├── manifest.json
│       └── resources/
...
`url(...)` reference points at a relative `resources/<filename>` path.
The plugin inlines sidecar CSS into `<style>` blocks before handing the
HTML to JCEF, because `<iframe srcdoc>` has no base URL and cannot
resolve relative paths on its own.

**`resources/screenshot.webp`** (or `.png`/`.jpeg`) — a visual reference
of the page at capture time. `resources/` may be omitted entirely if a
snapshot has no external sidecars to bundle.
...
version is not `2`. Older v1 bundles must be regenerated by re-running
your saver — see the v1→v2 migration section of
[`docs/snapshot-bundle-spec.md`](docs/snapshot-bundle-spec.md).

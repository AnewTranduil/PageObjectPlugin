...
- **Highlight current selector** — press `Alt+Shift+H` to highlight the locator on the current editor line in the snapshot.
...
- **Highlight All** — the tool window's **Show All** button highlights every locator in the current file at once with color-coded overlays and duplicate detection.

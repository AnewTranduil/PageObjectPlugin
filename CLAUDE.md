...
extracted as the framework-agnostic engine, the UI test suite runs under
...
- **Task 15 (Extract `@pagemirror/snapshot-core`):** the framework-agnostic
  engine ships as `packages/snapshot-core/` (`@pagemirror/snapshot-core`)
  and owns `assemble-html`, `manifest`, `save-snapshot`, and the
  `browser/collector` runtime script.
  `packages/playwright-snapshot-saver/` depends on it via semver and
  contributes only the Playwright-specific adapter, reporter, marker,
  trace pipeline, and source resolvers. The snapshot bundle format is
  ...
  resolve relative URLs). v1 bundles are refused and an in-tool-window
  banner directs users to `docs/migration-v1-to-v2.md` — regenerate via
...
- **Reference tests.** `tests/ToolWindowUiTest.kt` is the canonical
  ...
  enabled after `PageMirrorConfigurable` was given explicit accessible
  names on its testable fields and `PageMirrorLocators` was rewritten
  against those names.

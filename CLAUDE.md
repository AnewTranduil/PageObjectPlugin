...
packages/test-project/
  ...
  .snapshots/
    login/
      initial/        {index.html, manifest.json}                # no external CSS, so resources/ is omitted
      error-state/    {index.html, manifest.json}
    dashboard/
      initial/        {index.html, manifest.json, resources/}    # resources/<sha1>.css sidecars
...
the framework-agnostic engine behind it, the UI test suite runs under a
...
- **Task 15 (Extract `@pagemirror/snapshot-core`):** `packages/snapshot-core/` (published as `@pagemirror/snapshot-core`) owns bundle assembly, manifest building, and the browser-side collector; `packages/playwright-snapshot-saver/` is a thin Playwright adapter on top. This bumped the snapshot bundle format to **v2**: `screenshot.<ext>` lives under `resources/`, external CSS is written as `resources/<sha1>.css` sidecars referenced by `<link>`, and the plugin inlines sidecar CSS on read (since `srcdoc` iframes can't resolve relative URLs). v1 bundles are refused with a clear error message — regenerate via `npx playwright test` in `packages/test-project/`. See [`docs/migration-v1-to-v2.md`](docs/migration-v1-to-v2.md).

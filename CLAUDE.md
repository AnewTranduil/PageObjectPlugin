...
The `actions/` directory does **not** contain an `InsertLocatorAction` —
locator insertion is triggered as a side effect of the picker's result
callback, handled in `locators/PickerResultHandler.kt`. The action
surface is: `LoadSnapshotAction` (Tools menu), `ToggleInspectAction`
(`Alt+Shift+I`), and `HighlightCurrentSelectorAction` (`Alt+Shift+H`).

## Test Project Layout

The Playwright fixture project lives under `packages/` as part of the
npm workspace and is the source of truth for `.snapshots/` regeneration.
...
the `playwright-snapshot-saver` npm package is published on top of the
extracted `@pagemirror/snapshot-core` engine, the UI test suite runs
...
**Task 15 (Extract `@pagemirror/snapshot-core`)** — landed (PR #30
merged into main). The framework-agnostic core lives at
`packages/snapshot-core/` (published as `@pagemirror/snapshot-core`)
and the Playwright adapter at `packages/playwright-snapshot-saver/`
now delegates live capture to it. Both packages are wired into the
root npm workspace (see `package.json`). Task 15 also bumped the
...
test` in `packages/test-project/`. See

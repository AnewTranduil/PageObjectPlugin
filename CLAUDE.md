...
`playwright-snapshot-saver` npm package is published on top of the
extracted `@pagemirror/snapshot-core`, the UI test suite runs under a
layered Page Object structure, CI aggregates test results into a single
`claude-summary.{json,md}` bundle, and a Playwright-style trace viewer is
auto-generated on PRs tagged `demo`. The shipped plugin version is
**v0.5.0** (see `CHANGELOG.md`).
...
- **Task 15 (Extract `@pagemirror/snapshot-core`)** — merged. The
  `packages/snapshot-core/` package owns bundle assembly, manifest
  generation, save orchestration, and the browser-side collector;
  `playwright-snapshot-saver` is a thin adapter that depends on
  `@pagemirror/snapshot-core` via semver. The snapshot bundle format
  was bumped to v2: `screenshot.<ext>` moved under `resources/`, CSS is
  written as `resources/<sha1>.css` sidecars referenced by `<link>`,
  and the plugin inlines sidecar CSS on read (since `srcdoc` iframes
  can't resolve relative URLs). v1 bundles are refused with a
  user-visible banner — regenerate via `npx playwright test` in
  `packages/test-project/`. The v0.5.0 release added the
  **outdated-bundle banner** that surfaces this state in the tool
  window with a link to `docs/migration-v1-to-v2.md`.
- **Task 15.5 (Framework-agnostic trace rendering + resource inlining)** —
  merged. `@pagemirror/snapshot-core` now owns trace rendering behind a
...
**Build layout note.** Source packages live under `packages/` as npm
workspaces (`snapshot-core`, `playwright-snapshot-saver`, `test-project`)
— there is no top-level `test-project/` directory. The plugin
itself is a single Kotlin module rooted at the repository root, with
production code under `src/main/` and the package path
`com/github/artem/pageobjectplugin/`.

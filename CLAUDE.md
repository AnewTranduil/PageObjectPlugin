...
packages/test-project/
  ...
  fixtures/
    login.html
    app.html
    styles/{reset,theme,layout,components}.css
  page-objects/
    login.page.ts
    dashboard.page.ts
  tests/
...
Snapshots are generated by the `playwright-snapshot-saver` Playwright
reporter (`packages/playwright-snapshot-saver/src/reporter.ts`) driven by
`snapshot({ page, state })` markers inside the tests, not by a bespoke
`utils/save-state.ts` helper. Run `npm test --workspace packages/test-project`
(or `npx playwright test` inside the workspace) to regenerate the bundles.
...
split out as the framework-agnostic engine, the UI test suite runs under
a layered Page Object structure, CI aggregates test results into a single
`claude-summary.{json,md}` bundle, and a Playwright-style trace viewer is
auto-generated on PRs tagged `demo`. The monorepo is laid out as npm
workspaces (`package.json:workspaces = ["packages/*"]`) with
`snapshot-core`, `playwright-snapshot-saver`, and `test-project` as the
...
- **Task 15 (Extract `@pagemirror/snapshot-core`):** merged. The
  framework-agnostic core (`packages/snapshot-core/src/{assemble-html,
  manifest, save-snapshot, types}.ts` + `browser/collector.ts`) owns HTML
  assembly, manifest building, and the browser-side DOM collector.
  `playwright-snapshot-saver` depends on it via semver and ships the
  Playwright adapter (`playwright-adapter.ts`), the reporter, the
  `snapshot()` marker, the `extract` CLI, and the trace pipeline. The
  snapshot bundle format bumped to **v2**: `screenshot.<ext>` moved
  ...
  refused with an in-tool-window banner linking to

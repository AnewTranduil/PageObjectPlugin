...
snapshot bundle format has been bumped to v2 with a framework-agnostic
`@pagemirror/snapshot-core` engine underneath `playwright-snapshot-saver`,
the UI test suite runs under a layered Page Object structure, CI aggregates
...
**Task 15 (Extract `@pagemirror/snapshot-core`)** — complete and shipped
in plugin 0.5.0 / `playwright-snapshot-saver` 0.7.0 /
`@pagemirror/snapshot-core` 0.1.0. The snapshot bundle format is now v2:
...
relative URLs). `SnapshotBundle.SUPPORTED_BUNDLE_VERSION = 2` refuses v1
bundles and surfaces them through an outdated-bundle banner in the Page
Mirror tool window with a link to `docs/migration-v1-to-v2.md` —

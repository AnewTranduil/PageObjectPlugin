...
```json
{
  ...
  "timestamp": "2026-04-15T21:07:18.174Z",
  "playwright": "1.58.2",
  "userAgent": "Mozilla/5.0 ..."
}
```

`url` and `userAgent` are populated for live-capture snapshots
(`saveSnapshot`) and are optional for trace-extracted snapshots — the
Playwright trace may not carry them. The plugin reads `manifest.version`
and refuses to load unknown versions with a user-visible error surfaced
by the outdated-bundle banner in the tool window. See
`docs/snapshot-bundle-spec.md` for the authoritative schema.
...
extracted as a framework-agnostic core, the UI test suite runs under a
...
- **Task 13a (UI test unblock):** resolved via the `intellijPlatformTesting.runIde.register("runIdeForUiTests")` DSL in `build.gradle.kts` (around `:114`), which sidesteps the `splitMode` issue entirely.
...
- **Task 15 (Extract `@pagemirror/snapshot-core`):** shipped. `packages/snapshot-core/src/{assemble-html.ts, manifest.ts, save-snapshot.ts, browser/, trace/, types.ts}` own bundle assembly, manifest generation, and HTML post-processing. `playwright-snapshot-saver` depends on `@pagemirror/snapshot-core` and provides only the Playwright adapter (`playwright-adapter.ts`) plus the trace-extraction pipeline (`extractor.ts`, `reporter.ts`, `snapshot-marker.ts`, `sources/`, `trace/`). The snapshot bundle format is bumped to **v2**: `screenshot.<ext>` lives under `resources/`, CSS is written as `resources/<sha1>.css` sidecars referenced by `<link>`, and the plugin inlines sidecar CSS on read (since `srcdoc` iframes can't resolve relative URLs). v1 bundles are refused with a clear error surfaced via the outdated-bundle banner — regenerate via `npx playwright test` in `packages/test-project/`. See `docs/migration-v1-to-v2.md`.
- **Task 15.5 (Framework-agnostic trace rendering + resource inlining):** shipped. `@pagemirror/snapshot-core` owns trace rendering behind a `TraceBackend` interface (`packages/snapshot-core/src/trace/{types, renderer, inline, extract, runtime-script, content-type}.ts`). The Playwright package (`packages/playwright-snapshot-saver/src/trace/playwright-backend.ts`) reshapes `TraceLoader.storage()` into that interface; `extractor.ts` delegates to `extractFromBackend`. Trace bundles are fully self-contained — every `<link>`, `<img>`, CSS `url(...)`, `@font-face`, and SVG `<use>` reference points at a real file under `resources/`, and the `<base>` element is stripped. Selenium/Cypress/Appium adapters (Tasks 17, 20) will reuse `extractFromBackend` by implementing the same `TraceBackend` surface.
...
**In flight:** outdated-bundle banner + related v2 plumbing (see
`CHANGELOG.md` `[0.5.0]`) shipped in April 2026; no other Tasks are
currently in progress on the main branch.

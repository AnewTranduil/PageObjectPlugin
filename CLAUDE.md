...
Snapshots are produced by the `playwright-snapshot-saver` npm package
(reporter + `snapshot()` marker) rather than a hand-rolled
`utils/save-state.ts`.
...
published as the framework-agnostic engine, the UI test suite runs under
...
- **Task 15 (Extract `@pagemirror/snapshot-core`):** shipped as
  ...
  The snapshot bundle format is now **v2**: `screenshot.<ext>` lives
  ...
  refused with a clear outdated-bundle banner — regenerate via
  `npx playwright test` in `packages/test-project/`. Plugin release
  `0.5.0` ships this change.

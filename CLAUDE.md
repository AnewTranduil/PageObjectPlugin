...
Note: there is no `InsertLocatorAction` — locator insertion is wired
directly through `PickerResultHandler` when the user clicks an element
in the snapshot. The `HighlightCurrentSelectorAction` exposes the
caret-driven highlight path as an explicit action.
...
ship, the snapshot bundle format is on v2, the snapshot saver is split
into `@pagemirror/snapshot-core` + `playwright-snapshot-saver`, the UI
...
**Task 15 (Extract `@pagemirror/snapshot-core`)** — **complete.** The
framework-agnostic core lives at `packages/snapshot-core/` (published
as `@pagemirror/snapshot-core`). `packages/playwright-snapshot-saver/`
is now a thin Playwright adapter that depends on the core via semver
(`"@pagemirror/snapshot-core": "^0.1.0"`). Both packages are wired
together as npm workspaces at the repo root. Task 15 also bumped the
...
refused with a user-visible banner — regenerate via `npx playwright

...
packages/test-project/
  ...
  fixtures/
    app.html
    login.html
    styles/                       # CSS sidecars referenced by app.html
      components.css
      layout.css
      reset.css
      theme.css
...
extracted as a standalone npm package, the snapshot bundle format is at
v2, the UI test suite runs under a layered Page Object structure, CI
...
v0.5.0 / `playwright-snapshot-saver@0.7.0` /
`@pagemirror/snapshot-core@0.1.0`. The framework-agnostic engine lives
in `packages/snapshot-core/` (browser collector, HTML assembler,
manifest builder, save orchestration); `playwright-snapshot-saver`
became a thin Playwright adapter on top of it. The snapshot bundle
format bumped to v2: `screenshot.<ext>` moved under `resources/`, CSS
is written as `resources/<sha1>.css` sidecars referenced by `<link>`,
and the plugin inlines sidecar CSS on read (since `srcdoc` iframes
can't resolve relative URLs). `manifest.version` is the fixed schema
version `2`, not the incrementing write counter v1 carried. v1
bundles are refused with a clear error message and surface an
actionable banner in the Page Mirror tool window — regenerate via
`npx playwright test` in `packages/test-project/`. See
`docs/migration-v1-to-v2.md` for the migration guide.

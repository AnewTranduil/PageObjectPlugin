...
framework-agnostic bundle assembly and trace rendering, the UI test suite
...
- **Task 15 (Extract `@pagemirror/snapshot-core`):** the framework-agnostic
  core lives at `packages/snapshot-core/` and owns bundle assembly,
  manifest building, and the browser-side collector; the Playwright
  package (`packages/playwright-snapshot-saver/`) is now a thin adapter on
  top. This bumped the snapshot bundle format to v2: `screenshot.<ext>`
  ...
  are refused with a clear error message — regenerate via

...
1. **Scaffold** `packages/snapshot-core/` with `package.json` (name `@pagemirror/snapshot-core`), `tsconfig.json` matching the Playwright package, `src/`, `tests/`. Add to root `package.json` `workspaces` array. (As-shipped: the package is public — `"publishConfig": {"access": "public"}` — not `private: true`; the "make @pagemirror/snapshot-core publishable" commit switched it out of private mode.)
...
10. **Declare the dependency.** `packages/playwright-snapshot-saver/package.json` depends on `@pagemirror/snapshot-core`. (As-shipped: consumers use a real semver range — see the "switch consumers to semver" commit — so `@pagemirror/snapshot-core` can be published and installed the standard way. Do **not** use `workspace:*`.)
11. **Resolve `test-project/` dependency.** `packages/test-project/` participates in the root npm workspace and picks up both `playwright-snapshot-saver` and `@pagemirror/snapshot-core` via the workspace linkage. (Original plan proposed `file:` deps at repo root — superseded by the "consolidate packages under npm workspaces" refactor.)

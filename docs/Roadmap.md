...
Tasks 0–15.5 plus 19 are complete: the plugin works end-to-end for Playwright + TypeScript, the `@pagemirror/snapshot-core` + `playwright-snapshot-saver` npm packages ship snapshots from real Playwright runs, the settings UI is on Kotlin UI DSL v2, the UI test suite runs under a layered Page Object structure with polling/retry/trace-bundle diagnostics, CI aggregates unit + UI + Playwright results into a single `claude-summary.{json,md}` bundle consumed via `reports.artemon.cloud`, and PRs tagged `demo` auto-render a Playwright-style trace viewer. The framework-agnostic `@pagemirror/snapshot-core` package now owns bundle assembly and trace rendering (Tasks 15 + 15.5); `playwright-snapshot-saver` is a thin Playwright adapter on top. The IDE side is still tightly coupled to TypeScript (regex-based locator extraction). Tasks 16–18 + 20 below layer Selenium / Cypress / Appium / Python / JVM adapters on top of the extracted core without duplicating bundle assembly.
